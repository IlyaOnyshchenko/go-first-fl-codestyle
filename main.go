package main

import (
	"fmt"
	"math/rand"
	"strings"
)

const (
	warrior = "warrior"
	mage    = "mage"
	healer  = "healer"
	unknown = "неизвестный класс персонажа"
)

func randint(min, max int) int {
	return rand.Intn(max-min) + min
}
func harmDestiny(charName string, harm int) string {
	return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, harm)
}
func attack(charName, charClass string) string {
	switch charClass {
	case warrior:
		return harmDestiny(charName, 5+randint(3, 5))
	case mage:
		return harmDestiny(charName, 5+randint(5, 10))
	case healer:
		return harmDestiny(charName, 5+randint(-3, -1))
	default:
		return unknown
	}
}
func harmBlock(charName string, harm int) string {
	return fmt.Sprintf("%s блокировал %d урона.", charName, harm)
}

// обратите внимание на "if else" и на "else"
func defence(charName, char_class string) string {
	switch char_class {
	case warrior:
		return harmBlock(charName, 10+randint(5, 10))
	case mage:
		return harmBlock(charName, 10+randint(-2, 2))
	case healer:
		return harmBlock(charName, 10+randint(2, 5))
	default:
		return unknown
	}
}
func specialSkill(charName, spSkill string, health int) string {
	return fmt.Sprintf("%s применил специальное умение `%s` %d", charName, spSkill, health)
}

// обратите внимание на "if else" и на "else"
func special(charName, charClass string) string {
	switch charClass {
	case warrior:
		return specialSkill(charName, `Выносливость`, 80+25)
	case mage:
		return specialSkill(charName, `Атака`, 5+40)
	case healer:
		return specialSkill(charName, `Защита`, 10+30)
	default:
		return unknown
	}
}

// здесь обратите внимание на имена параметров
func startTraining(charName, charClass string) string {
	switch charClass {
	case warrior:
		fmt.Printf("%s, ты Воитель - отличный боец ближнего боя.\n", charName)
	case mage:
		fmt.Printf("%s, ты Маг - превосходный укротитель стихий.\n", charName)
	case healer:
		fmt.Printf("%s, ты Лекарь - чародей, способный исцелять раны.\n", charName)
	}

	fmt.Println("Потренируйся управлять своими навыками.")
	fmt.Println("Введи одну из команд: attack — чтобы атаковать противника,")
	fmt.Println("defence — чтобы блокировать атаку противника,")
	fmt.Println("special — чтобы использовать свою суперсилу.")
	fmt.Println("Если не хочешь тренироваться, введи команду skip.")

	var cmd string
	for cmd != "skip" {
		fmt.Print("Введи команду: ")
		fmt.Scanf("%s\n", &cmd)
		switch cmd {
		case "attack":
			fmt.Println(attack(charName, charClass))
		case "defence":
			fmt.Println(defence(charName, charClass))
		case "special":
			fmt.Println(special(charName, charClass))
		}
	}
	return "тренировка окончена"
}

// обратите внимание на имя функции и имена переменных
func choiseCharClass() string {
	approveChoice, charClass := "", ""
	for approveChoice != "y" {
		fmt.Print("Введи название персонажа, за которого хочешь играть: Воитель — warrior, Маг — mage, Лекарь — healer: ")
		fmt.Scanf("%s\n", &charClass)
		switch charClass {
		case warrior:
			fmt.Println("Воитель — дерзкий воин ближнего боя. Сильный, выносливый и отважный.")
		case mage:
			fmt.Println("Маг — находчивый воин дальнего боя. Обладает высоким интеллектом.")
		case healer:
			fmt.Println("Лекарь — могущественный заклинатель. Черпает силы из природы, веры и духов.")
		}
		fmt.Print("Нажми (Y), чтобы подтвердить выбор, или любую другую кнопку, чтобы выбрать другого персонажа: ")
		fmt.Scanf("%s\n", &approveChoice)
		approveChoice = strings.ToLower(approveChoice)
	}
	return charClass
}

// обратите внимание на имена переменных
func main() {
	fmt.Println("Приветствую тебя, искатель приключений!")
	fmt.Println("Прежде чем начать игру...")

	charName := ""
	fmt.Print("...назови себя: ")
	fmt.Scanf("%s\n", &charName)

	fmt.Printf("Здравствуй, %s\n", charName)
	fmt.Println("Сейчас твоя выносливость — 80, атака — 5 и защита — 10.")
	fmt.Println("Ты можешь выбрать один из трёх путей силы:")
	fmt.Println("Воитель, Маг, Лекарь")

	charClass := choiseCharClass()

	fmt.Println(startTraining(charName, charClass))
}
